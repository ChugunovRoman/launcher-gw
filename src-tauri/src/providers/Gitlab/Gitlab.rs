...
use crate::{
  ...
  providers::{
    ...
    Gitlab::{files::*, group::*, issues::*, launcher::*, release::*, repo::*},
    dto::{Issue, *},
  },
...
impl ApiProvider for Gitlab {
  ...
  fn get_asset_url(&self) -> String {
    format!("{}/projects/<PROJECT_ID>/packages/generic/<NAME_SPACE>/<VERSION>/<FILE_NAME>", self.host)
  }
  ...
  async fn create_tag(&self, repo_id: &str, tag_name: &str, branch: &str) -> Result<()> {
    __create_tag(self, repo_id, tag_name, branch).await
  }
