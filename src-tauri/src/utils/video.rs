...
pub fn get_available_resolutions() -> Result<Vec<String>, String> {
  let event_loop = winit::event_loop::EventLoop::new()
    .map_err(|e| format!("Не удалось создать EventLoop: {}", e))?;

  let primary_monitor = event_loop
    .primary_monitor()
    .ok_or("Основной монитор не найден")?;
  ...
  for mode in primary_monitor.video_modes() {
    ...
    if seen.insert(key) {
      resolutions_with_data.push((
        mode,
        format!("{}x{} ({}Hz)", size.width, size.height, refresh_rate),
      ));
    }
  }
