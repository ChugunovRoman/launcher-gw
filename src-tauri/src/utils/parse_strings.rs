...
pub fn extract_total(input: &str) -> Result<(u32, usize)> {
  ...
  let bytes = captures.get(2).unwrap().as_str().parse::<usize>()?;

  Ok((files, bytes))
}

pub fn extract_output(input: &str) -> Result<usize> {
  let re = Regex::new(r"Archive size: (\d+) bytes")?;
  let captures = re.captures(input).ok_or_else(|| anyhow!("Строка не соответствует ожидаемому формату"))?;

  let bytes = captures.get(1).unwrap().as_str().parse::<usize>()?;
